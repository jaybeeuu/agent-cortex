...
8. Hand over to your harness's completion-detection mechanism: Copilot keeps a `sleep 120` poll timer running in the background; PI keeps a  call outstanding over every in-flight agent ID. If no chore was dispatched and HITL gate beads are pending (`bd list -l lifecycle:feature-pr -l implementation-type:hitl` or `bd list -l awaiting-epic-pr-merge`), go straight to HITL Pause (REFERENCE.md).
...
2. Read the completed agent's output with read_agent and parse its `---REPORT---` block for `BEAD_ID`, `STAGE_COMPLETED`, and the outcome.
...
- Spawning a stage runner via bash or running a stage inline instead of task.
...
- [ ] Every stage runner was spawned with task from the parent's feature worktree, with the `---REPORT---` instruction in its prompt.
