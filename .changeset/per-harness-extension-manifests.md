...
deterministic plan without spawning anything. Because the manifest now owns the
third-party pi packages, they are removed from the `pi/settings.json` template —
re-run `agent-cortex ext install --harness pi` after `agent-cortex install pi` to
re-register them. The npm `postinstall` hook is unchanged, and agent-cortex's own
bundled extensions stay out of the manifests (they ride pi package discovery).
