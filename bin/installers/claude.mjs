 ...
 */
function readMarketplaceManifestFile(root) {
 ...
 */
function readMaterialisedPluginVersion(root, manifest) {
  const source = manifest.plugins?.[0]?.source;
  if (typeof source !== "string") return undefined;
  const pluginDir = source.startsWith(".") ? join(root, source) : resolve(source);
  try {
    return JSON.parse(readFileSync(join(pluginDir, ".claude-plugin", "plugin.json"), "utf-8")).version;
  } catch {
    return undefined;
  }
}
...
function queryClaudeCommand(claudeBin, args) {
  const label = `${claudeBin} ${args.join(" ")}`;
  const result = spawnSync(claudeBin, args, { encoding: "utf8" });
  if (result.error) {
    throw new Error(`failed to run "${label}": ${result.error.message}`);
  }
 ...
 */
function marketplaceRegistered(claudeBin, marketplace) {
  const out = queryClaudeCommand(claudeBin, ["plugin", "marketplace", "list", "--json"]);
 ...
 */
function installedPluginVersion(claudeBin, pluginId) {
  const out = queryClaudeCommand(claudeBin, ["plugin", "list", "--json"]);
...
export function registerClaude({
  ...
  const { manifest: manifestObj, path: manifestPath } =
    manifest !== undefined ? { manifest, path: null } : readMarketplaceManifestFile(root);
  ...
  const version = pluginVersion ?? readMaterialisedPluginVersion(root, manifestObj);
  ...
  if (probe.error || probe.status !== 0) {
    const why = probe.error
      ? `not found on PATH: ${probe.error.message}`
      : `predates the "plugin" subcommand (exit ${probe.status}) — update Claude Code (v2+)`;
    warn(
      `Claude Code CLI "${claudeBin}" ${why} — the plugin is materialised but NOT registered with Claude Code. ` +
  ...
  const marketPresent = marketplaceRegistered(claudeBin, marketplace);
  const installed = installedPluginVersion(claudeBin, pluginId);
