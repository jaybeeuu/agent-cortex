...
- Agents live in composable `<name>/` directories under `agents/`: a shared `agent.md` body plus
  ...
  `claude/` subtree is generated by `scripts/build-claude-agents.mjs` (`ralph` is authored natively
  in `agents-native/`; the other agents compose from their `claude/` harness dirs).
