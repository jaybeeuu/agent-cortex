...
- `bin/installers/claude.mjs` (plain `agent-cortex install claude` materialises the plugin
  into `~/.agent-cortex/claude` and registers it with Claude Code; `pnpm build:claude` is its
  generate-only alias, running `install claude --output claude`) composes the plugin's
...
Code) and its generate-only `pnpm build:claude` alias (`install claude --output claude`)
— the committed `claude/` subtree is regenerated by the alias and drift-checked, so it can
never be stale; `copilot` ships via
