...
- Agents live in composable `<name>/` directories under `agents/`: a shared `agent.md` body plus
  ...
  `claude/` subtree is generated by the shared `bin/installers/claude.mjs` code path behind both
  `agent-cortex install claude` and `scripts/build-claude-agents.mjs` (thin wrapper; `ralph` is
  authored natively in `agents-native/`; the other agents compose from their `claude/` harness dirs).
