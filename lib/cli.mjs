...
// Options accepted by `ext install`. `--harness` is required (validated by
// validateExtHarness); `--dry-run` is shared with the installers.
const EXT_INSTALL_OPTIONS = {
  "--dry-run": { key: "dryRun", boolean: true },
  "--harness": { key: "harness" },
};
  ...
  if (command === "ext") {
    ...
    if (subcommand !== "install") {
      return {
        command: "ext",
        subcommand,
        optionError: `Unknown ext subcommand "${subcommand}". Only "install" is supported. Run "agent-cortex --help" for usage.`,
      };
    }
    ...
    for (let i = 0; i < extRest.length; i++) {
      ...
      const spec = inlineValue !== null ? EXT_INSTALL_OPTIONS["--harness"] : EXT_INSTALL_OPTIONS[arg];
      if (!spec) {
        return { command: "ext", subcommand: "install", optionError: `Unknown option "${arg}". Run "agent-cortex --help" for usage.` };
      }
      if (spec.boolean) {
        options[spec.key] = true;
        continue;
      ...
      if (value == null || value.startsWith("--")) {
        return { command: "ext", subcommand: "install", optionError: `Missing value for "${arg}". Run "agent-cortex --help" for usage.` };
      }
      options[spec.key] = value;
      i += 1;
    }

    return { command: "ext", subcommand: "install", ...options };
  }
