...
- `scripts/build-copilot-agents.mjs` (`pnpm build:copilot`) composes `copilot/agents/*.agent.md`
  from each agent's `copilot/` harness dir (alongside `copilot/plugin.json`, the
  token-substituted `copilot/skills/` tree, and `copilot/hooks.json`) — committed so Copilot CLI
  can load the subtree directly.
- `bin/installers/claude.mjs` (plain `agent-cortex install claude` materialises the plugin
  into `~/.agent-cortex/claude` and registers it with Claude Code; `--output <dir>` is its
  generate-only form, used by tests/CI) composes the plugin's
  `agents/*.md` from each agent's `claude/` harness dir (except `ralph`, which ships natively
  from `agents-native/ralph.md`, copied verbatim).
...
Never edit the generated output by hand — edit the composable directory.
...
and CI validates the materialiser with structural checks on a temp-dir install; `copilot` ships as
the committed `copilot/` subtree, generated by `bin/installers/copilot.mjs`, the shared
code path behind both `agent-cortex install
