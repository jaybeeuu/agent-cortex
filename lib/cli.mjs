...
Commands:
  install <harness>   Install agent-cortex for a harness
                      ...
                      Options (claude):
                        (no --output)   Generate the plugin subtree AND register it
                                        with Claude Code via the "claude plugin" CLI
                                        (marketplace add → install → update) — requires
                                        the plugin CLI (Claude Code v2+)
                        --output <dir>  Write the plugin subtree to <dir> only — no
                                        runtime registration (default: <package
                                        root>/claude)
