...
- `bin/installers/claude.mjs` (plain `agent-cortex install claude` also registers the plugin
  with Claude Code; `pnpm build:claude` is its generate-only alias, running `install claude
  --output claude`) composes `claude/agents/*.md` from each agent's `claude/` harness dir (except
...
(which also registers the plugin with Claude Code) and its generate-only `pnpm
build:claude` alias (`install claude --output claude`) — the committed `claude/` subtree
is regenerated by it and drift-checked, so it can never be stale; `copilot` ships via
