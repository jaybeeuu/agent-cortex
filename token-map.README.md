...
- `scripts/build-copilot-agents.mjs` — the copilot harness token composer
  (scripts/lib/compose-agent.mjs) resolves `{{TOOL:...}}` / `{{PATH:...}}` against
  the map and `{{SECTION:...}}` from the per-harness section files.
- `bin/installers/claude.mjs` — the single Claude generator, behind both
  `agent-cortex install claude` and the `pnpm build:claude` alias (no build-time
  counterpart; the committed `claude/` subtree is regenerated by it and drift-checked).
