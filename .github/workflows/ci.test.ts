...
describe("Publish workflow", () => {
  ...
  it("sets up Node with npm registry", () => {
    ...
    const with_ = (setupNode as Record<string, unknown>).with as Record<
      ...
      unknown
    >;
    assert.equal(with_["registry-url"], "https://registry.npmjs.org");
  });
  ...
  it("publishes to npm", () => {
    ...
    assert.ok(run.includes("pnpm publish"), "runs pnpm publish");
  });

  it("uses NODE_AUTH_TOKEN from secrets", () => {
    const steps = getSteps(publish);
    const publishStep = steps.find(
      (s: Record<string, unknown>) => s.name === "Publish to npm"
    );
    assert.ok(publishStep, "has Publish to npm step");
    const env = (publishStep as Record<string, unknown>).env as Record<
      string,
      unknown
    >;
    assert.equal(env.NODE_AUTH_TOKEN, "${{ secrets.NPM_TOKEN }}");
  });
});
