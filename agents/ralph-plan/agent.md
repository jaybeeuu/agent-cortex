...
If the request is ambiguous or incomplete, use {{TOOL:ask_user}} to resolve blockers before exploring. One question at a time.
...
- Use {{TOOL:bash}} only for read-only commands (`git log`, `find`, `cat`, etc.).
