...
**Amendment (2026-08-30):** see `self-flushing-session` — the sharper mechanism record for this
principle: agent autonomously decides when to flush, beads *are* the memory store (no separate
system), a parent session bead holds the conversation with child outcome beads discovered
progressively, and the bead ID is the only thing kept across a flush. This record remains the
principle; `self-flushing-session` is the mechanism.