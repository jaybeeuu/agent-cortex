...
- `bin/installers/claude.mjs` — the single Claude generator, behind plain
  `agent-cortex install claude` (materialises the plugin into `~/.agent-cortex/claude`
  with token-substituted skills and registers it with Claude Code) and its generate-only
  `pnpm build:claude` alias (`install claude --output claude`; no build-time counterpart —
  the committed `claude/` subtree is regenerated by it and drift-checked).
