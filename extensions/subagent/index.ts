...
export default function (pi: ExtensionAPI) {
	...
	// ── Background agent runs for Copilot-compatible task/read_agent tools ──
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			cwd: Type.Optional(
				Type.String({ description: "Working directory for the sub-agent (default: current project)" }),
			),
		}),
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			// Create a minimal agent config with no system prompt override
	...
	pi.registerTool({
		name: "read_agent",
		label: "Read Background Agent Output",
		description:
			"Read the output of a background agent spawned with the task tool. " +
			"Returns the full output if the agent has completed, or 'STILL RUNNING' if not yet finished.",
		parameters: Type.Object({
			agentId: Type.String({ description: "Agent ID returned by the task tool" }),
		}),
